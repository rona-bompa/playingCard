...
class PlayingCardView: UIView {
    ...
    private func centeredAttributedString(_ string: String, fontSize: CGFloat) -> NSAttributedString {
        ...
        font = UIFontMetrics(forTextStyle: .body).scaledFont(for: font) // IMPORTANT line if we do fonts
    ...
    private func createCornerLabel() -> UILabel {
        let label = UILabel()
        label.numberOfLines = 0 // it means use as many lines as you need
        addSubview(label)
        return label
    }
    ...
    private func configureCornerLabel(_ label: UILabel) {
        ...
        label.frame.size = CGSize.zero // I am going to clear size before sizeToFit()
        label.sizeToFit()
        label.isHidden = !isFaceUp
    }

    // when we flip the screen, the font/view redraws (with the new font size)
...
//        path.fill()
        ...
        if isFaceUp {
            ...
            if let cardBackImage = UIImage(named: "cardBackground", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
